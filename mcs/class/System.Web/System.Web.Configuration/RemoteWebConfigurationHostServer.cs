...
namespace System.Web.Configuration {
    ...
    public class RemoteWebConfigurationHostServer : IRemoteWebConfigurationHostServer
    {
        ...
        public void GetFileDetails(string name, out bool exists, out long size, out long createDate, out long lastWriteDate) {
            ...
            FileAttributesData fileData;
            var result = FileAttributesData.GetFileAttributes(name, out fileData);
            if (result == HResults.S_OK)
            {
                exists = true;
                size = fileData.FileSize;
                createDate = fileData.UtcCreationTime.Ticks;
                lastWriteDate = fileData.UtcLastWriteTime.Ticks;
            }
            else {
                exists = false;
                size = 0;
