...
namespace System.Web.Management {
    ...
    public sealed class RegiisUtility : IRegiisUtility {
        ...
        private void DoKeyCreate(string containerName, string csp, long options, int keySize)
        {
            ...
            uint returnHR = (uint)AlternativeNativeMethods.DoesKeyContainerExist(containerName, csp, ((options & DO_RSA_PKU) == 0) ? 1 : 0);
            switch (returnHR) {
                case 0:
        ...
        private void DoKeyAclChange(string containerName, string account, string csp, long options)
        {
            ...
            if ((options & DO_RSA_FULL_ACCESS) != 0)
                flags |= 0x4;
            int returnHR = AlternativeNativeMethods.ChangeAccessToKeyContainer(containerName, account, csp, flags);
            if (returnHR != 0)
        ...
        private static void MakeSureContainerExists(string containerName, string csp, bool machineContainer) {
            uint returnHR = (uint) AlternativeNativeMethods.DoesKeyContainerExist(containerName, csp, machineContainer ? 1 : 0);
            switch (returnHR) {
                case 0:
                    return; // success!
                case 0x80090016:
